...
async function showCreateSchemaInput() {
    let result = await window.showInputBox({
        ...
        validateInput: text => {
            return isValidURL(text) ? null : 'Invalid URL: Has to start with http or https';
        }
    }).then((value) => {
        if(value !== undefined){	
            if(workspace.workspaceFolders !== undefined){
                ...
                let schema = util.getSchemaFromEndpoint(value)
                    .catch(error => window.showErrorMessage('Error creating Schema:'+ error))
                    .then(() => {
                        var openPath = Uri.file(join(pathToFolder.uri.fsPath, 'graphqlschema', 'schema.gql'));
...
export const createSchemaCommand = commands.registerCommand('extension.createSchema', () => {
