...
const { introspectionQuery, buildClientSchema, printSchema, GraphQLSchema } = require("graphql");
...
export default class GraphQLUtils{
    ...
    public async getSchemaFromEndpoint(endpoint:string){
        ...
        if(!response){
            ...
            throw new Error('Schema fetching went wrong, could not execute introspection query');
        }
        ...
        try{
            ...
            fs.rmdir(this.folder);
            throw new Error('Schema fetching went wrong, response did not contain any body');
        }
        ...
        if(!body.data || !body.data.__schema){
            fs.rmdir(this.folder);
            throw new Error('Schema fetching went wrong, submitted url is no graphql endpoint');
        }
        ...
        //write json file
        await fs.writeFile(resultpath, JSON.stringify(body.data, null, 2), 'utf-8'); 
        
        try{
            //Create schema from data
            let schema = buildClientSchema(body.data as IntrospectionQuery);
            //Save schema as gql file
            await fs.writeFile(path.join(this.folder,'schema.gql'), printSchema(schema), 'utf-8');
